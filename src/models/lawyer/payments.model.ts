...
const lawyer_invoice_model = pgTable(
  "lawyer_invoice",
  {
    invoice_number: varchar({ length: 15 }).primaryKey(),
    issuer_id: bigint({ mode: "number" })
      .references(() => user_model.id, { onDelete: "cascade" })
      .notNull(),
    client_id: bigint({ mode: "number" }).references(() => user_model.id, {
      onDelete: "set null",
    }),
    client_name: varchar({ length: 50 }).notNull(),
    client_phone: varchar({ length: 30 }),
    client_email: varchar({ length: 50 }),
    case_reference: varchar({ length: 30 }).references(() => case_model.id, {
      onDelete: "set null",
    }),
...
export type {
  LawyerInvoiceType,
  InsertLawyerInvoiceType,
  UpdateLawyerInvoiceType,
};
