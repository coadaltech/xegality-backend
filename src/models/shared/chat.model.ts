...
const messages_model = pgTable('messages', {
  ...
  from: bigint({ mode: "number" }).references(() => user_model.id, { onDelete: 'cascade' }).notNull(),
  to: bigint({ mode: "number" }).references(() => user_model.id, { onDelete: 'cascade' }).notNull(),
  content: text(),
...
export type { MessageType, InsertMessageType, UpdateMessageType, UserConnectionType, InsertUserConnectionType, UpdateUserConnectionType };
