...
import { pgTable, bigint, integer, varchar, char, text, real, date, jsonb, boolean } from "drizzle-orm/pg-core";
...
const student_profile_model = pgTable("students", {
  ...
  // Application
  practice_area_interests: varchar({ enum: PRACTICE_AREAS_CONST }).array().notNull(),
...
type StudentWithUserType = StudentType & Omit<InferSelectModel<typeof user_model>, 'id' | 'password' | 'role' | 'created_at' | 'refresh_token'>;
type UpdateStudentWithUserType = Partial<StudentWithUserType>;

export { student_profile_model };
export type { StudentType, InsertStudentType, UpdateStudentType, StudentWithUserType, UpdateStudentWithUserType };
