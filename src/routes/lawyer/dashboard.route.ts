...
import { CaseSchema, PRIRORITY_CONST, STATUS_CONST } from "../../types/case.types";
import { get_case_details, get_cases_list } from "../../services/shared/case.service";
import { create_new_case, get_cases_history, get_connected_consumers, update_case } from "../../services/lawyer/dashboard.service";
import { create_invoice, get_invoices_list, get_invoice_details, update_invoice, delete_invoice, update_invoice_status } from "../../services/lawyer/payment.service";
...
const lawyer_dashboard_routes = new Elysia({ prefix: "/lawyer/dashboard" })
  ...
  .get("/fetch-invoice-details/:invoiceNumber", async ({ set, store, params }) => {
    const invoice_response = await get_invoice_details(params.invoiceNumber, store.id);
    ...
    {
      params: t.Object({
        invoiceNumber: t.String({ description: "Invoice number to fetch details for" })
      })
    }
  )

  .delete("/delete-invoice/:invoiceNumber", async ({ set, store, params }) => {
    const delete_response = await delete_invoice(params.invoiceNumber, store.id);

    set.status = delete_response.code;
    return delete_response;
  },
    { params: t.Object({ invoiceNumber: t.String({ description: "Invoice number to delete" }) }) }
  )

  .put("/update-invoice-status/:invoiceNumber", async ({ set, store, params, body }) => {
    const update_status_response = await update_invoice_status(params.invoiceNumber, body.status, store.id);

    set.status = update_status_response.code;
    return update_status_response;
  },
    {
      params: t.Object({ invoiceNumber: t.String({ description: "Invoice number to update status for" }) }),
      body: t.Object({ status: t.String({ description: "New status for the invoice" }) })
    }
  )
